...
public class HouseServiceImpl implements HouseService {
    ...
    public JSON speInfo(String house_id_hash) {
        ...
        try {
            //下载上线和下线概要房屋信息表
            isi.download(fht1.getPath(),"QmRK8cwQRgMoyhopH4YxXbY84JVsu4uuF6r7pf8Scq9rEA","");
            isi.download(fht2.getPath(),"QmRK8cwQRgMoyhopH4YxXbY84JVsu4uuF6r7pf8Scq9rEA","");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        //存放得到的房子的图片
        String[] house_pic = new String[dhl.length-2];
    ...
    @Override
    //搜索
    public JSON search(JSONObject low_location, String lease_inter, String house_type, String lease_type, boolean elevator) {
        ...
        lowlocation.StoLL2(low_location);
        ...
        try {
            //下载在线房屋概要信息表
            isi.download(fht1.getPath(), "Qmf4Kui6PVPsMtRYphDxWR3jnNrjkdij8uvwZetYetHiSx", "");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        //如果实参有具体值，则记录该实参在
        if (!lowlocation.getProvi().equals("0")) {
            valued.add(0, 1);
            count++;
        } else if (lowlocation.getProvi().equals("0")) {
            valued.add(0, 0);
        }
        if (!lowlocation.getCity().equals("0")) {
            valued.add(1, 1);
            count++;
        } else if (lowlocation.getCity().equals("0")) {
            valued.add(1, 0);
        }
        ...
        if (!lowlocation.getCommu_name().equals("0")) {
            valued.add(3, 1);
            count++;
        } else if (lowlocation.getCommu_name().equals("0")) {
            valued.add(3, 0);
        }
...
//        System.out.print(h.allInfo());
        ...
        IPFS_SERVICE_IMPL i=new IPFS_SERVICE_IMPL();
        String p2="E:\\houseleasing\\houseleasing\\src\\main\\file\\housetable3.txt";
